function [M,N,P,R,x_coarse,y_coarse,F_coarse,x_fine,y_fine,F_fine] = zadanie5()
 % Parametry generacji węzłów interpolacji
    close all
    M = 16; % liczba węzłów interpolacji wzdłuż osi X (stopień wielomianu zmiennej x: M-1)
    N = 16; % liczba węzłów interpolacji wzdłuż osi Y (stopień wielomianu zmiennej y: N-1)
    P = 105; % liczba punktów wzdłuż osi X na gęstszej siatce
    R = 105; % liczba punktów wzdłuż osi Y na gęstszej siatce
