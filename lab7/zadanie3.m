function [integration_error, Nt, ft_5, integral_1000] = zadanie3()
    ...
    function integral = simpson_integration(f, a, b, k)
        delta = (b - a) / k;
        integral = 0;
        for i = 1:k
            x = a + (i - 1) * delta;
            x_next = a + i * delta;
            integral = integral + (f(x) + 4*f((x_next + x)/2) + f(x_next));
        end
        integral = (delta/6) * integral;
    end

    % Wyznaczenie wartości całki dla N=1000
    integral_1000 = simpson_integration(f, 0, 5, 1000);
    
    % Pętla po różnych liczbach podprzedziałów całkowania
    for z = 1:length(Nt)
        N = Nt(z);
        integration_result = simpson_integration(f, 0, 5, N);
